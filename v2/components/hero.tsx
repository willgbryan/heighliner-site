...
export function Hero() {
  ...
  return (
    ...
    >
      ...
      <div className="text-balance relative z-20 mx-auto mb-4 mt-4 max-w-5xl text-left text-5xl pl-4 font-semibold tracking-tight text-stone-900 dark:text-neutral-300 md:text-7xl">
        <Balancer>
          <motion.h2>
            {"Legal research starts here."
              .split(" ")
              .map((word, index) => (
                <motion.span
                  initial={{
      ...
      <motion.p
        ...
        className="relative z-20 mx-auto mt-4 max-w-2xl px-4 text-left text-base/6 text-gray-600 dark:text-gray-200"
      >
        Our first of it's kind legal search engine finds compelling sources, builds reports, and engineers boolean searches to
        empower your legal research.
      </motion.p>
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
  ...
  return (
    <>
      <motion.div
        ...
        className={cn(
          "absolute left-96 top-20 m-auto h-14 w-px rounded-full bg-gradient-to-t from-slate-500 via-slate-500 to-transparent",
  ...
  return (
    <div {...props} className={cn("absolute z-50 h-2 w-2", props.className)}>
      <motion.div
        ...
        className="absolute -inset-x-10 top-0 m-auto h-[4px] w-10 rounded-full bg-gradient-to-r from-transparent via-slate-500 to-transparent blur-sm"
      ></motion.div>
      {spans.map((span) => (
        <motion.span
          key={span.id}
          ...
          className="absolute h-1 w-1 rounded-full bg-gradient-to-b from-slate-500 to-slate-500"
        />
      ))}
    </div>
  );
